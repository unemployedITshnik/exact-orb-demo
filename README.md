...
Каталог `ephe/` не хранится в репозитории (`.gitignore`). Без файлов эфемерид
`swisseph` уходит в fallback-режим Moshier — расчёт не падает, но точность и
набор тел ниже, и в карту попадают предупреждения.

Нужны три файла с сайта Astrodienst
...
`EXACT_ORB_EPHE_PATH` → `[tool.exact_orb].ephemeris_path` в `pyproject.toml` →
`data/ephe`. Текущий проектный конфиг указывает на `ephe`.
...
| `--house-system` | `P` | односимвольный код системы домов Swiss Ephemeris (`P`, `K`, `O`, `R`, `C`, `E`, `W`) |
...
**аргумент функции → переменная окружения → `[tool.exact_orb]` в ближайшем
`pyproject.toml` → значение по умолчанию в коде.**
...
так что установка пакета для прогона не обязательна.
...
docs/
  ...
  requirements/decisions/      ADR-0001 … ADR-0020
...
ephe/             файлы Swiss Ephemeris (не в репозитории)
