...
**Расширение ADR-0032.**
`ResolvedBirthData` внутри состояния получает
`birth_time_domain: BirthTimeDomain | None`. Для любого явно указанного
времени, включая `12:00`, значение `None`; для космограммы домен обязателен.
Это производный разрешённый факт, но он участвует в расчётном намерении:
`matches_intent` продолжает сравнивать весь `birth_resolved`, поэтому два
разных домена не считаются одним намерением.
...
Транзиты в первый срез не входят, поэтому потребителям доступна только
`base_chart`; отдельное поле `active_view` не материализуется. Сужение
временное и снимается вместе с транзитами; оформляется ревизией ADR-0016.
...
State/dialog JSON имеют отдельные payload versions. Новые state-записи имеют
payload v2, dialog остаётся v1. Совместимость state v1 закреплена frozen
fixture с relational metadata: decoder читает обе state-версии. При чтении
state v1 известное время получает
`birth_time_domain=None`; для `time_unknown=true` migration-helper
birth-слоя строит домен из сохранённых `birth_input.birth_date` и
`birth_resolved.tz_id`. SQLite adapter выбирает decoder по версии, но не
реализует IANA/timezone policy. Мигрированное состояние записывается как v2
при следующей мутации. Не разрешившаяся дата или зона дают типизированный
persistence failure, а не повторное использование полуденной космограммы.
Frozen fixture v1 не изменяется; отдельный golden v2 фиксирует новые записи.
SQL schema и dialog payload version не меняются.
